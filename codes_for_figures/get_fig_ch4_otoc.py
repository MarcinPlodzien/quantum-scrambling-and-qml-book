...
Produces the OTOC figure for Chapter 4, contrasting chaotic and integrable
spin-chain dynamics:
...
equally.  We use it because:
    ...
    (b) It equals the Hilbert-Schmidt inner product ⟨A, B⟩ = Tr(A†B)/D,
        ...
        (Chapter 4, Sec. 4.5).  This makes the comparison direct.
...
The two figures in Chapter 4 tell a complementary story:
    • fig_ch4_krylov.pdf:  Intrinsic diagnostic (no choice of V needed)
    • fig_ch4_otoc.pdf:    Extrinsic diagnostic (depends on V), but
...
FILES
===============================================================================
    Caches data to:   data/ch4/otoc_data_t15.npz
    Saves figure to:  figures/ch4/fig_ch4_otoc.pdf  (+.png at 300 dpi)
===============================================================================
"""
...
DATA_DIR = ROOT / "data" / "ch4"
FIG_DIR  = ROOT.parent / "figures" / "ch4"
...
# =============================================================================
def compute():
    """Expensive numerics: exact-diagonalization OTOC heatmaps C_{0j}(t) for the
    chaotic (MFI) and integrable (TFI) chains.  Cached via load_or_compute."""
    D = 2**N_SPIN
    s = np.arange(D, dtype=np.int32)
    ...
    return {"times": times, "map_chaotic": map_chaotic,
            "map_integrable": map_integrable}


def main():
    data = load_or_compute(DATA_DIR / "fig_ch4_otoc.npz", compute)
    times          = data["times"]
    ...
    panel_label(ax_a, "a", loc="upper left", color="white")
    ax_a.text(0.14, 0.96, "Chaotic (MFI)", transform=ax_a.transAxes,
    ...
    ax_b.text(0.14, 0.96, "Integrable (TFI)", transform=ax_b.transAxes,
              va="top", ha="left", color="white", fontweight="bold")
    ...
    out_path = FIG_DIR / "fig_ch4_otoc.pdf"
