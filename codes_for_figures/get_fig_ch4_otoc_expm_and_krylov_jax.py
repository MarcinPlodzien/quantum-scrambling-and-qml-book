...
Produces OTOC heatmaps C_{0j}(t) for Chapter 4, contrasting chaotic and
...
where α is the Lanczos coefficient slope.  The two Chapter 4 figures tell
complementary stories:
    • fig_ch4_krylov.pdf:  Intrinsic diagnostic (no choice of V needed)
    • fig_ch4_otoc.pdf:    Extrinsic, spatially-resolved spreading
...
def run_simulation_for_N(N):
    ...
    # Plain-numpy .npz cache, keyed on the parameter-encoding filename.
    data = load_or_compute(data_path, compute)
    times = data["times"]
    ...
    ax_a.set(xlabel="Time $t$", ylabel="Probe site $r$", yticks=site_labels)
    panel_label(ax_a, "a", loc="upper left", color="white")
    ax_a.text(0.14, 0.96, rf"Chaotic (MFI)  $h_x={H_X},\, h_z={H_Z_CHAOTIC}$",
              transform=ax_a.transAxes, va="top", ha="left", color="white", fontsize=9)

    im_b = ax_b.pcolormesh(times, site_labels, map_integrable.T, shading="auto", cmap="inferno", vmin=0, vmax=vmax)
    ax_b.set(xlabel="Time $t$", ylabel="Probe site $r$", yticks=site_labels)
    panel_label(ax_b, "b", loc="upper left", color="white")
    ax_b.text(0.14, 0.96, rf"Integrable (TFI)  $h_x={H_X},\, h_z={H_Z_INTEGRABLE}$",
              transform=ax_b.transAxes, va="top", ha="left", color="white", fontsize=9)
