#!/usr/bin/env python3
"""
Generate Random Matrix Theory (RMT) figures for Chapter 2: Random Matrix Theory
...
Figures produced
────────────────
1. fig_ch2_level_spacing.pdf / .png
...
Usage:
    python get_fig_ch2.py               # generate data (if not cached) + plot
    python get_fig_ch2.py --regenerate  # force data regeneration
    python get_fig_ch2.py --plot-only   # skip generation, use cached data
...
OUTPUT_DIR = SCRIPT_DIR.parent / "figures" / "ch2"
DATA_DIR = SCRIPT_DIR / "data" / "ch2"
...
# Ensemble registry: name -> (sampler, spacing extractor, is_gse).
ENSEMBLES = {
    "GOE": (sample_goe, get_spacings,     False),
    "GUE": (sample_gue, get_spacings,     False),
    "GSE": (sample_gse, get_spacings_gse, True),
}


def compute_spacings(name):
    """
    Diagonalize N_SAMPLES matrices for one ensemble and extract unfolded
    spacings.  Returns a dict cached to data/ch2/spacings_{name}.npz.
    """
    sampler, spacer, _ = ENSEMBLES[name]
    print(f"  Generating {name} spacings ({N_SAMPLES} × {D}×{D})...")
    all_spacings = []
    for i in range(N_SAMPLES):
        H = sampler(D)
        eigs = np.linalg.eigvalsh(H)
        all_spacings.extend(spacer(eigs))
    return {"spacings": np.array(all_spacings), "D": D, "N_SAMPLES": N_SAMPLES}


def compute_eigenvalues(name):
    """
    Diagonalize N_SAMPLES matrices for one ensemble and collect all eigenvalues.
    The semicircle law is universal across GOE/GUE/GSE.  Returns a dict cached to
    data/ch2/eigenvalues_{name}.npz.
    """
    sampler, _, is_gse = ENSEMBLES[name]
    ...
    return {"eigenvalues": np.array(all_eigs), "D": D, "N_SAMPLES": N_SAMPLES}
...
def plot_level_spacing():
    ...
    for ax, (plabel, display_name, file_key, beta, color) in zip(axes, panels):
        # Load cached data (or compute + cache on a miss)
        data = load_or_compute(DATA_DIR / f"spacings_{file_key}.npz",
                               partial(compute_spacings, file_key))
        ...
        ax.text(0.97, 0.95, f"{plabel} {display_name}",
    ...
    outpath = OUTPUT_DIR / "fig_ch2_level_spacing.pdf"
...
def plot_semicircle():
    ...
    for ax, (plabel, display_name, file_key, color) in zip(axes, panels):
        data = load_or_compute(DATA_DIR / f"eigenvalues_{file_key}.npz",
                               partial(compute_eigenvalues, file_key))
    ...
    outpath = OUTPUT_DIR / "fig_ch2_semicircle.pdf"
    fig.savefig(outpath, bbox_inches="tight")
    fig.savefig(outpath.with_suffix(".png"), bbox_inches="tight")
    print(f"    → Saved: {outpath}")
    plt.close(fig)
...
def plot_eigenvalue_sticks():
    ...
    outpath = OUTPUT_DIR / "fig_ch2_level_repulsion.pdf"
    fig.savefig(outpath, bbox_inches="tight")
    fig.savefig(outpath.with_suffix(".png"), bbox_inches="tight")
    print(f"    → Saved: {outpath}")
    plt.close(fig)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Chapter 2 — RMT Figure Generation")
    print(f"  D = {D},  N_SAMPLES = {N_SAMPLES}")
    print("=" * 60)

    # Expensive diagonalizations are cached via load_or_compute inside the
    # plotting routines (set FIG_RECOMPUTE=1 to force regeneration).
    print("\n── Generating figures ──")
    apply_book_style()
    plot_level_spacing()
    plot_semicircle()
    ...
    # Human-readable CSV summaries (reads the .npz caches the plots just built).
