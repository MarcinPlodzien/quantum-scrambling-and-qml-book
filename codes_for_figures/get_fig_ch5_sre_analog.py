...
═══════════════════════════════════════════════════════════════════════════════
    ...
    python get_fig_ch5_sre_analog.py                 # compute (or load cache) + plot
    FIG_RECOMPUTE=1 python get_fig_ch5_sre_analog.py  # force the merged cache to rebuild
...
def compute():
    """Ensure the per-N M₂(t) caches exist, then merge them into one dict.

    The expensive Trotter evolution + FWHT SRE numerics live in
    generate_data(); it writes/loads the per-N caches (sre_analog_N*.npz)
    that get_fig_ch5_sre_scaling.py also consumes, so those are preserved.
    """
    generate_data(regenerate=False)
    out = {}
    for N in N_VALUES:
        d = np.load(DATA_DIR / f"sre_analog_N{N}.npz")
        out[f"times_N{N}"] = d["times"]
        out[f"sre_N{N}"] = d["sre"]
    return out
...
def plot_sre(data):
    ...
    ax.set_ylabel(r"Stabilizer Rényi Entropy $M_2$")
...
data = load_or_compute(DATA_DIR / "fig_ch5_sre_vs_time.npz", compute)
