...
===================================
FAST VERSION of the Krylov complexity figure generator for Chapter 4.
...
FILES
============================================================
Caches data to:  data/ch4/krylov_data.npz   (avoids recomputation)
Saves figure to: figures/ch4/fig_ch4_krylov.pdf
"""
...
def compute():
    """Expensive numerics: operator Lanczos + C_K(t).  Cached via load_or_compute."""
    D = 2**N_SPIN
    ...
    return {"b_cha": b_cha, "times": times, "CK_cha": CK_cha, "N_spin": N_SPIN}


def main():
    data = load_or_compute(DATA_DIR / "fig_ch4_krylov.npz", compute)
    b_cha  = data["b_cha"]
    times  = data["times"]
    CK_cha = data["CK_cha"]
    ...
    ax = axes[0]
    # Show the FULL computed range so that both the linear-growth regime and the
    # finite-N saturation of b_n are visible. Cropping to the linear window alone
    # would present a saturating sequence as if it grew linearly without end.
    n_show = len(b_cha)
    n_cha = np.arange(1, n_show + 1)

    # Fit line (black dashed): drawn ONLY over the fit window [8,30] where b_n is
    # linear, not extended across the saturated tail.
    n_line = np.linspace(0, 32, 100)
    fit_line = np.maximum(alpha_bn * n_line + gamma_bn, 0)
    ax.plot(n_line, fit_line, "--k", lw=2.2, alpha=0.7, zorder=1)

    # Data points
    ax.plot(n_cha, b_cha[:n_show], "s", ms=2.5, color="#d7191c",
    ...
    # ── Save ──────────────────────────────────────────────────────────────
    out_path = FIG_DIR / "fig_ch4_krylov.pdf"
