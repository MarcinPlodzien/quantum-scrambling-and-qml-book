...
the spatially-resolved OTOC data computed by get_fig_ch4_otoc.py.
...
In the heatmap C_{0j}(t) plotted by get_fig_ch4_otoc.py:
...
from plot_style import apply_book_style, panel_label
...
# =============================================================================
def main():
    # Reuse the OTOC heatmap cached by get_fig_ch4_otoc.py (the expensive
    # numerics live there; this script's own work — threshold extraction — is
    # trivial, so it needs no separate cache).
    cache_path = DATA_DIR / "fig_ch4_otoc.npz"
    if not cache_path.exists():
        print(f"ERROR: cached data not found at {cache_path}")
        print("Run get_fig_ch4_otoc.py first to generate the heatmap data.")
        return
    ...
    panel_label(axes[0], "a", loc="upper left")
    axes[0].text(0.14, 0.96, "Chaotic (MFI)", transform=axes[0].transAxes,
                 va="top", ha="left")
    panel_label(axes[1], "b", loc="upper left")
    axes[1].text(0.14, 0.96, "Integrable (TFI)", transform=axes[1].transAxes,
                 va="top", ha="left")

    out_path = FIG_DIR / "fig_ch4_otoc_v_butterfly.pdf"
